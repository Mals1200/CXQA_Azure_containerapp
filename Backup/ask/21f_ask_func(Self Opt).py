# version 21f:
#########################################################################################
# OPTIMIZATION NOTICE - v11b
#
# The following optimizations have been applied to this file:
#
# 1. Persistent HTTP Sessions:
#    - All OpenAI/Azure API calls now use a single requests.Session() to reuse connections,
#      improving speed for each API request (no repeated SSL/TLS handshakes).
#
# 2. In-Memory Caching for RBAC & Schema Loads:
#    - The RBAC (User/File access) and table schema/sample loading functions now use
#      @lru_cache to cache results, avoiding repeated blob downloads and Excel/CSV parsing.
#    - Cache can be refreshed manually via .cache_clear() if RBAC or schema files are updated.
#
# 3. Parallel Execution of Index Search and Python Tool:
#    - The AI Index Search (Tool-1) and Python Data Tool (Tool-2) are now run in parallel
#      using ThreadPoolExecutor, reducing the time to answer multi-source questions.
#
# 4. Phase Timing Logging:
#    - Timing for major steps (API calls, tools, etc.) is now printed in the logs for
#      easier performance debugging and bottleneck identification.
#
# 5. Fastest LLM for Splitters/Classifiers:
#    - The fastest available LLM deployment (e.g., GPT-4o or GPT-3.5-turbo) is now used
#      for lightweight classifier/splitter calls, improving speed without affecting answer quality.
#
# IMPORTANT:
# - No business logic, access control, or answer generation code has been altered.
# - All core functionality, robustness, and output structure remain exactly the same.
...
CONFIG = {
    ...
    "LLM_ENDPOINT_AUX" : "https://malsa-m3q7mu95-eastus2.cognitiveservices.azure.com/openai/deployments/gpt-4o/chat/completions?api-version=2025-01-01-preview",
...
def call_llm(system_prompt, user_prompt, max_tokens=500, temperature=0.0):
    ...
    try:
        ...
        # Step 1 Optimization: Use persistent HTTP session for API calls
...
def agent_answer(user_question, user_tier=1, recent_history=None):
    ...
    # Step 3 Optimization: Run Index and Python Tools in Parallel
    with concurrent.futures.ThreadPoolExecutor() as executor:
        fut_python = executor.submit(tool_2_code_run, user_question, user_tier, recent_history) if needs_tabular_data else None
        fut_index  = executor.submit(tool_1_index_search, user_question, 5, user_tier)
        index_dict = fut_index.result()
        python_dict = fut_python.result() if fut_python else {"result": "No information", "code": ""}
    t1 = time.time()
    print(f"[TIMING] Index and Python tools took {t1-t0:.2f} seconds")
...
def Ask_Question(question, user_id="anonymous"):
    ...
    try:
        ...
        answer_collected = ""
        try:
            # Step 4 Optimization: Log the time each phase takes
            t0 = time.time()

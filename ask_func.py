...
def execute_generated_code(code_str):
    ...
    try:
        ...
        # Replace read operations in the generated code with the dataframes we loaded
...
def Ask_Question(question):
    ...
    # NEW CHECK: If user requests "export ppt", call the PPT_Agent function
    if "export ppt" in question.lower():
        from PPT_Agent import Call_PPT

        # Find the most recent non-"export ppt" user question (if any)
        last_user_question = None
        last_assistant_answer = None
        for i in reversed(range(len(chat_history))):
            entry = chat_history[i]
            if entry.startswith("User:") and "export ppt" not in entry.lower():
                last_user_question = entry[5:].strip()
                break
        # Find the most recent assistant answer
        for i in reversed(range(len(chat_history))):
            entry = chat_history[i]
            if entry.startswith("Assistant:"):
                last_assistant_answer = entry[10:].strip()
                break

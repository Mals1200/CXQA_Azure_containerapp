...
def clean_main_answer(answer_text):
    """
    1. If answer_text is a JSON object with a "content" array, flatten it into plain text.
    2. Otherwise (or afterward), remove every line that begins with:
       - "Source:" (case‐insensitive)
       - "Referenced:" (case‐insensitive)
       - "Calculated using:" (case‐insensitive)
    """
        ...
        try:
            ...
            if isinstance(response_json, dict) and isinstance(response_json.get("content"), list):
                ...
                for block in response_json["content"]:
                    # Each block is usually a dict like { "type": "...", "text": "..." }
                    if isinstance(block, dict):
                        text_val = block.get("text", "").strip()
                        if text_val:
                            md_lines.append(text_val)
                # Return flattened text (no JSON keys remain, hence no "Source:" from JSON)
                return "\n\n".join(md_lines).strip()
        except Exception:
            # If JSON parsing fails, simply fall back to stripping below
            pass

    # ——— Step 2: Remove any LLM-provided footers ———
    # We scan line by line and drop
    #   * any line starting with "Source:"
    #   * any line starting with "Referenced:"
    #   * any line starting with "Calculated using:"
    #
    # This also removes those labels plus all lines AFTER them (if you prefer).
    #
    ...
    for line in lines:
        # If we encounter a footer label, we skip it and everything after.
        if re.match(r'^\s*(Source:|Referenced:|Calculated using:)', line, flags=re.IGNORECASE):
            skip_footer = True
            continue

        if skip_footer:
            # Once we've seen "Source:" or "Referenced:" or "Calculated using:",
            # ignore all subsequent lines too.
            continue
